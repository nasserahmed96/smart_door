...
void recvFile(int fd){
	...
	printf("file size: %s\n", buffer);
	/* Where *socket_fd is the socket file descriptor in the client side
	 * buffer is the buffer where to save the data into which is (char *)
	 * file_size is the recieved file size in bytes*/
	 

/*	if((recv_size = read(*socket_fd, buffer, file_size)) == 0){
		printf("Error in recieving file: %s\n", gai_strerror(errno));
		exit(1);
	}
	while(recv_size != file_size && sent_size != 0){
		fwrite(file_recv, buffer, file_size);
	}*/
	/* saving recieved data to the device
	 * where buffer is the buffer conatins the file data onyl
	 * recv_size the data recieved
